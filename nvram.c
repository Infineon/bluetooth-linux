...
FILE * config_get_file(uint16_t config_item_id, char * read_option)
{
    ...
    if (!p_file)
    {
        debug_PrintError("[%s] Unable to open NVRAM file %s with %s\n", __FUNCTION__, nvram_file_name, read_option);
        return NULL;
    }
...
uint32_t config_VS_Read (uint16_t config_item_id, uint32_t len, uint8_t *buf)
{
    ...
    debug_Printf("[%s] id 0x%x len %u\n", __FUNCTION__, config_item_id, read_len);
    return read_len ;
}
...
uint32_t config_VS_Write(uint16_t config_item_id, uint32_t len, uint8_t* buf)
{
    ...
    debug_Printf("[%s] id 0x%x len %u\n", __FUNCTION__, config_item_id, write_len);
    return write_len;
}
