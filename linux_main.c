...
#define SPY_TRACE_TYPE_SCO_RX 40
#define SPY_TRACE_TYPE_SCO_TX 41
...
static BOOL32 cy_platform_open_file_write(const char *file_name, const char* content)
{
    ...
    if(fd == -1)
    {
        debug_PrintError("opening %s failed.\n", file_name);
        return FALSE;
    }
    else
    {
...
static BOOL32 cy_platform_gpio_write(const char *dev_name, const char* offset, uint8_t value, char *str)
{
    ...
    debug_Printf("[%s]:Write value %d to GPIO at offset %s (OUTPUT mode) on chip %s\n", __FUNCTION__,value, offset, dev_name);
    ...
    if (ret == -1)
    {
        debug_PrintError("[%s]:Unable to line handle from ioctl : %s\n", __FUNCTION__, strerror(errno));
        return FALSE;
    }
    data.values[0] = value;
    ...
    if (ret == -1)
    {
        debug_PrintError("[%s]:Unable to set line value using ioctl : %s\n", __FUNCTION__, strerror(errno));
        return FALSE;
    }
...
{
    debug_Printf("Bluetooth_linux porting layer version:%s\n", BLUETOOTH_LINUX_VER);
    debug_Printf("[%s]: patchfile %s, device %s, baud %d\n", __FUNCTION__, patchFile, device, baudrate);
    ...
    if (device[0] == 'C' || device[0] == 'c' || strncmp("/dev/",device,5) == 0)
    {
        ...
        {
            debug_PrintError("could not set up serial port %s at %d init_baud_rate - exiting!!!\n", device, (patchFile[0] == '\0') ? app_baud_rate : 115200);
            return FALSE;
        }
        bUseUART = TRUE;
    }
    ...
    {
        bUseUART = FALSE;
        debug_PrintError("USB Not supported   !!!\n");
        return FALSE;
    }
    ...
    {
        debug_PrintError("pthread_create for timer_task failed\n");
        return FALSE;
    }
...
static void SendHciTraceToSpy(wiced_bt_hci_trace_type_t type, uint16_t len, uint8_t *p_data)
{
    ...
    if (type == HCI_TRACE_EVENT)
        ...
        debug_PrintError ("SendHciTraceToSpy - unknown type: %d\n", type);
}
...
{
    debug_PrintError("!!!! EXCEPTION Code: 0x%x  Message: %s ptr:%x\n", code, msg, ptr);
}
