...
static void print_usage( char *p_app_name )
{
    ...
    debug_Printf( "Usage:\n" );
    debug_Printf( "\n" );
    debug_Printf( "%s -c <comport> -b <baud rate> -d <bda> -i <spy instance>\n", p_app_name );
    debug_Printf( "\nFor example:\n" );
    debug_Printf( "   %s -c COM21 -b 3000000 -i 1 -p \"firmware.hcd\")\n", p_app_name );
    debug_Printf( "\n" );
    debug_Printf( " c  Local device name, for example COM21 for UART.\n" );
    debug_Printf( " b  Baud rate for HCI UART device. support 115200 and 3000000 only\n" );
    debug_Printf( " d  Bluetooth Device Address to be programmed"
                    " to the device. A pre-determined address"
                    " is used if this is omitted.\n" );
    debug_Printf( " i  BT Spy instance number for logging (default is 0)\n" );
    debug_Printf( " r  GPIO bank and pin number for BR_REG_ON\n" );
    debug_Printf( " p  Full file path of .hcd patch file to download to device on startup.\n" );
    debug_Printf( " f  baud rate to be used for patch download, default value is 115200 if not specified, support 115200 and 921600 only\n" );
    debug_Printf( " t  Provide the IP address\n" );
    debug_Printf( " s  Enable TCP Socket for data transfer\n" );
    debug_Printf( "\n");
    debug_Printf( " NOTE: Please provide instance before enabling TCP"
                        ...
                        cybt_controller_gpio_config_t *p_gpio_cfg)
{
    ...
    for ( int i = 1; i < argc; i++ )
    {
        ...
        case 'C': /* HCI UART Port */
            ...
            {
                debug_PrintError( "No comport found in Command line options.\n" );
                print_usage( app_name );
                return PARSE_ERROR;
            }
            break;
        ...
        case 'B': /* HCI baud rate */
            ...
            {  
                ...
                {
                    debug_PrintError( "set baud rate error in Command line options.\n" );
                    print_usage( app_name );
                    return PARSE_ERROR;
                }
            }
            ...
            {
                debug_PrintError( "No baud rate found in Command line options.\n" );
                print_usage( app_name );
                return PARSE_ERROR;
            }
            break;
        case 'F':     /* FW patch download baud rate */
            ...
            {
                debug_PrintError( "No baud rate for patch download found in Command"
                                                        "line options.\n" );
                print_usage( app_name );
                return PARSE_ERROR;
            }
        ...
        case 'P':     /* FW patch file to download */
            ...
            {
                debug_PrintError( "No patch file found in Command line options.\n" );
                print_usage( app_name );
                return PARSE_ERROR;
            }
            break;
        ...
        case 'D': /* Device BD Address */
            ...
            {
                debug_PrintError( "No BDA found in Command line options.\n" );
                print_usage( app_name );
                return PARSE_ERROR;
            }
            break;
