...
void patch_download_check (void)
{
    ...
    {
        debug_Printf("Downloading patchfile %s\n", PATCH_FILE_NAME);
        ...
        if ((fd = fopen(PATCH_FILE_NAME, "rb")) != NULL)
        {
            ...
            if ((patch_info.btewiced_patchram_len > 0) && (patch_info.p_btewiced_patchram_buf = (uint8_t *)malloc(patch_info.btewiced_patchram_len)) != NULL)
            {
                ...
                if  (read_len > 0)
                {
                    debug_Printf("Downloading patchfile %s (size: %i)\n", p_file, patch_info.btewiced_patchram_len);
                ...
                {
                    debug_PrintError("reading patch file\n");
                    free(patch_info.p_btewiced_patchram_buf);
                }
            }
            else
            {
                debug_PrintError("Unable to buffer to hold patchram (%i bytes), or invalid patch file\n", patch_info.btewiced_patchram_len);
            }
            fclose(fd);
        ...
        {
            debug_PrintError("Unable to open patchfile %s\n", PATCH_FILE_NAME);
        }
    }
    else{
        debug_Printf("Proceeding to post reset..\n");
...
static void patch_download_complete(tBRCM_PRM_STATUS status)
{
    ...
    if (status == BRCM_PRM_STS_COMPLETE)
    {
        ...
        if(bUseUART)
        {
           if(patch_baud_rate != DEFAULT_PATCH_BAUD_RATE)
           {
                ...
                {
                        debug_PrintError("Uart reconfiguring is failed\n");
                        return;
                }
            }
        ...
        debug_Printf("Sending UpdateBaudRateVSC \n");
    ...
    {
        debug_PrintError("Patch downloading is failed status=0x%x\n", status);
    }
}
...
static void update_baudrate_cback (tBTM_VSC_CMPL* p)
{
    debug_Printf("%s: result = %X\n",__FUNCTION__, p->p_param_buf[0]);
    debug_Printf("%s: changed baudrate to %d\n",__FUNCTION__, app_baud_rate);
...
static void signal_controller_reset (void)
{
    debug_Printf("HCI reset has been sent");
    pthread_mutex_lock(&mutex_controller_reset);
